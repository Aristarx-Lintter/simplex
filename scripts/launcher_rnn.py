...
def main():
    ...
    for i, run_cfg in enumerate(sweep_params):
        experiment_name = f"run_{i}_L{run_cfg['model_config']['num_layers']}_H{run_cfg['model_config']['hidden_size']}_{run_cfg['model_config']['rnn_type']}_{'bi' if run_cfg['model_config'].get('bidirectional', False) else 'uni'}_{run_cfg['process_config']['name']}"
