...
def load_process_data(config, process_dir, generate_data=True):
    ...
    if not os.path.exists(data_dir):
        ...
        if generate_data:
            print(f"Generating data for {process_string}")
            generate_and_save_data(config, process_dir)
        else:
            print(f"Data directory {data_dir} does not exist and generate_data is False")
            return None
...
def get_process_string(process_config, n_ctx, bos):
    sorted_keys = sorted(process_config)
    process_string = "_".join(f"{key}_{process_config[key]}" for key in sorted_keys if key != 'name')
...
def generate_and_save_data(config, output_dir):
    ...
    seq_lens = config['model_config']['n_ctx']
    n_ctxs = [seq_lens] if isinstance(seq_lens, int) else seq_lens
